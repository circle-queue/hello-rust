...
def get_main_loop_path(input: list[str]) -> list[tuple[int, int]]:
    ...
    for i, line in enumerate(input):
        for j, char in enumerate(line):
            ...
            if char == "S":
                start = (i, j)
                continue
            for i_, j_ in dirs[char]:
                G[i, j].append((i + i_, j + j_))
    G = dict(G)
    ...
    node_from = start
    node = next(k for k, v in G.items() if start in v)
    stack = [node]
    while node != start:
        # print(node_from, node)
        i, j = node
        node1, node2 = [(i_, j_) for i_, j_ in G[i, j]]
        if node1 == node_from:
            next_node = node2
        elif node2 == node_from:
            next_node = node1
        else:
            raise ValueError
        node_from, node = node, next_node
        stack.append(node)

    G[start] = [stack[0], stack[-2]]
    return G, stack


def solve1(input: list[str]) -> str:
    G, loop = get_main_loop_path(input)
...
def solve2(input: list[str]) -> str:
    ...
    # Determine the symbol of start
    a = loop[0]
    start = loop[-1]
    b = loop[-2]
    delta_1 = (a[0] - start[0], a[1] - start[1])
    delta_2 = (b[0] - start[0], b[1] - start[1])
    start_symb = next(
        k for k, v in dirs.items() if v == (delta_1, delta_2) or v == (delta_2, delta_1)
    )
    dirs_ = dirs | {"S": dirs[start_symb]}

    # Double grid size
    flood = {(i * 2, j * 2) for (i, j) in loop}
    for i, j in loop:
        for i_, j_ in dirs_[input[i][j]]:
            flood.add((i * 2 + i_, j * 2 + j_))

    orig_flood = flood.copy()
    max_i = max(i for i, j in flood)
    max_j = max(j for i, j in flood)

    def _solve(flood_start: tuple[int, int], flood: set) -> int:
        todo = [flood_start]
        while todo:
            i, j = now = todo.pop()
            if now in flood:
            ...
            for i_, j_ in itertools.product(range(i - 1, i + 2), range(j - 1, j + 2)):
                if (i_, j_) in flood or i_ < 0 or j_ < 0 or i_ > max_i or j_ > max_j:
                    continue
                todo.append((i_, j_))

        flood -= orig_flood
        internal_tiles = sum([1 for i, j in flood if i % 2 == j % 2 == 0])

        return internal_tiles

    for i, j in loop:
        if input[i][j] == "F":
            # Could also be on one of two side, i.e.
            flood_start_1 = (i * 2 + 1, j * 2 + 1)  # Bottom right
            flood_start_2 = (i * 2 - 1, j * 2 - 1)  # Top left
            sol1 = _solve(flood_start_1, flood.copy())
            sol2 = _solve(flood_start_2, flood.copy())

            # We could be confident about our solution by
            # traversing the loop and adding both sides of the loop to two sets
            # The smaller set is the inner side, which is the place to start

            # This is true for the sample and test, but not in general
            return str(min(sol1, sol2))
...
sample_input2_3 = """FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
full_input1 = full_input2 = """..FL-77L|7FL--J.F.77.F--F7J7FF-JJ.F-7J77.LJ7.L77F7F.F7777FFJ---7-77.FF-|.7---L7FL.-J.L-7F-J77FL7FF----|----J.FLF-F7.F-F-LJ..-FJJ7.FJ-7J-FJFL
...
FJLLJL---|L7J-LLJ.J.--JJ.7..F7J.J-LF-JJ.J--7JJ|JL7JLF..LJJ-LFJJ.-JL7L-JJ.LLJLL.L-J-LLJJ-JLJ-JJJ-F--LF-J-LL-L7LJ.L-L|7-7JLLJLJLJJJ7JL-L7L|JJJ""".splitlines()
