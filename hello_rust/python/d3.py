...
def sol1():
    ...
    for i, row in enumerate(x.splitlines()):
        for j, c in enumerate(row):
            if c.isdigit() or c == '.':
                continue
            valid_cells |= {(I, J) for I in range(i-1, i+2) for J in range(j-1, j+2)}
...
def sol2():
    ...
    for i, row in enumerate(x.splitlines()):
        for nums_span in re.finditer(r'\d+', row):
            ...
            for I, J in ((I, J) for I in range(i-1, i+2) for J in range(j_start-1, j_end+1)):
                valid_nums[I, J].append(int(nums_span.group()))
